 ...
 * Converts PascalCase keys from C# to camelCase for TypeScript
 */
export async function loadManifest(): Promise<TestManifest> {
  ...
  return {
    ...
    wmlTests: (raw.WmlTests || []).map(toCamelCaseTest),
    smlTests: (raw.SmlTests || []).map(toCamelCaseTest),
    pmlTests: (raw.PmlTests || []).map(toCamelCaseTest),
  };
}

function toCamelCaseTest(obj: Record<string, unknown>): Record<string, unknown> {
    ...
    if (Array.isArray(value)) {
      result[camelKey] = value.map(item =>
        typeof item === 'object' && item !== null
          ? toCamelCaseTest(item as Record<string, unknown>)
          : item
      );
    } else {
