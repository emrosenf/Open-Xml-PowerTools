...
impl SmlDataRetriever {
    pub fn get_sheet_path(sml_doc: &SmlDocument, sheet_name: &str) -> Result<String> {
        ...
        let sheets_elem = workbook_doc
            .elements_by_name(workbook_root, &XName::new(S::NS, "sheets"))
            .next()
            .ok_or_else(|| RedlineError::InvalidPackage {
        ...
        // Find specific sheet
        let sheet_elem = workbook_doc
            .elements_by_name(sheets_elem, &XName::new(S::NS, "sheet"))
            .find(|&s| {
                workbook_doc
                    .get(s)
                    ...
                    .map_or(false, |attrs| {
                        attrs
                            .iter()
                            .any(|a| a.name.local_name == "name" && a.value == sheet_name)
                    })
            })
            .ok_or_else(|| RedlineError::InvalidPackage {
                message: format!("Invalid sheet name: {}", sheet_name),
            })?;

        // Get r:id
        let r_id = workbook_doc
            .get(sheet_elem)
            .and_then(|d| d.attributes())
            .and_then(|attrs| attrs.iter().find(|a| a.name == R::id()))
            .map(|a| a.value.clone())
            .ok_or_else(|| RedlineError::InvalidPackage {
                message: "Sheet has no r:id".to_string(),
            })?;

        // Resolve relationship
        let workbook_rels_path = "xl/_rels/workbook.xml.rels";
        let workbook_rels_doc = pkg.get_xml_part(workbook_rels_path)?;
        let rels_root = workbook_rels_doc
            .root()
            .ok_or_else(|| RedlineError::InvalidPackage {
                message: "Workbook rels has no root".to_string(),
            })?;

        let rel_elem = workbook_rels_doc
            .elements_by_name(
                rels_root,
                &XName::new(
            ...
            .find(|&r| {
                workbook_rels_doc
                    ...
                    .map_or(false, |attrs| {
                        attrs
                            .iter()
                            .any(|a| a.name.local_name == "Id" && a.value == r_id)
                    })
            })
            .ok_or_else(|| RedlineError::InvalidPackage {
        ...
        let target = workbook_rels_doc
            .get(rel_elem)
            .and_then(|d| d.attributes())
            .and_then(|attrs| attrs.iter().find(|a| a.name.local_name == "Target"))
            .map(|a| a.value.clone())
            .ok_or_else(|| RedlineError::InvalidPackage {
                message: "Relationship has no Target".to_string(),
            })?;
    ...
    fn retrieve_range_impl(
        ...
        let sh_root = sh_xdoc.root().ok_or_else(|| RedlineError::InvalidPackage {
        ...
        if let Some(sheet_data_id) = sheet_data_elem {
            let out_sheet_data_id =
                out_doc.add_child(out_root_id, XmlNodeData::element(S::sheetData()));

            let row_name = S::row();
            for row_id in sh_xdoc.elements_by_name(sheet_data_id, &row_name) {
                let r_attr_val = sh_xdoc
                    .get(row_id)
                    .and_then(|d| d.attributes())
                    .and_then(|attrs| attrs.iter().find(|a| a.name.local_name == "r"))
                    .map(|a| a.value.as_str());

                if r_attr_val.is_none() {
                    continue;
                }
                ...
                for cell_id in sh_xdoc.elements_by_name(row_id, &c_name) {
                    let cell_addr_val = sh_xdoc
                        .get(cell_id)
                        .and_then(|d| d.attributes())
                        .and_then(|attrs| attrs.iter().find(|a| a.name.local_name == "r"))
                        .map(|a| a.value.clone());

                    if cell_addr_val.is_none() {
                        continue;
                    }
                                ...
                                if let Ok(idx) = val.parse::<usize>() {
                                    if let Some(ref sst) = sst_xdoc {
                                        if let Some(sst_root) = sst.root() {
                                            ...
                                            // Find si by index
                                            let si_node =
                                                sst.elements_by_name(sst_root, &si_name).nth(idx);
                    ...
                    let v_name = S::v();
                    let v_val = sh_xdoc
                        .elements_by_name(cell_id, &v_name)
                        .next()
                        .and_then(|v| {
                            sh_xdoc.children(v).find_map(|c| {
                                sh_xdoc
                                    .get(c)
                    ...
                    if let Some(attrs) = cell_data.attributes_mut() {
                        ...
                        attrs.push(XAttribute::new(
                            XName::new("", "ColumnNumber"),
                            &col_index.to_string(),
                        ));

                        if !cell_type.is_empty() {
                            let type_val = if cell_type == "inlineStr" {
                                "s"
                            } else {
                                cell_type
                            };
                        ...
                        // Formula
                        if let Some(f) = sh_xdoc
                            .get(cell_id)
                            .and_then(|d| d.attributes())
                            .and_then(|a| a.iter().find(|x| x.name.local_name == "f"))
                        {
                            attrs.push(XAttribute::new(XName::new("", "Formula"), &f.value));
                        }
                        // Style
                        if let Some(s) = sh_xdoc
                            .get(cell_id)
                            .and_then(|d| d.attributes())
                            .and_then(|a| a.iter().find(|x| x.name.local_name == "s"))
                        {
                ...
                if !out_cells.is_empty() {
                    ...
                    for (cell_data, val, disp_val) in out_cells {
                        ...
                        let val_id = out_doc
                            .add_child(cell_id, XmlNodeData::element(XName::new("", "Value")));
                        out_doc.add_child(val_id, XmlNodeData::text(&val));

                        let disp_val_id = out_doc.add_child(
                            cell_id,
                            XmlNodeData::element(XName::new("", "DisplayValue")),
                        );
...
mod xlsx_tables {
    ...
    pub fn parse_range(range: &str) -> Result<(i32, i32, i32, i32)> {
        ...
        if parts.len() != 2 {
            return Err(RedlineError::InvalidPackage {
                message: "Invalid range format".to_string(),
            });
        }
    ...
    pub fn split_address(address: &str) -> Result<(String, String)> {
        ...
        if i == 0 || i == address.len() {
            return Err(RedlineError::InvalidPackage {
                message: "Invalid cell address".to_string(),
            });
        }
...
mod tests {
    ...
    fn test_split_address() {
        assert_eq!(
            xlsx_tables::split_address("A1").unwrap(),
            ("A".to_string(), "1".to_string())
        );
        assert_eq!(
            xlsx_tables::split_address("AA10").unwrap(),
            ("AA".to_string(), "10".to_string())
        );
    }
}
