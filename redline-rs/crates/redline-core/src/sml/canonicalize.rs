...
use crate::sml::signatures::{
    WorkbookSignature, WorksheetSignature, CellSignature, CellFormatSignature,
    CommentSignature, DataValidationSignature, HyperlinkSignature,
};
use crate::xml::{XmlDocument, XName, S, R};
...
impl SmlCanonicalizer {
    ...
    pub fn canonicalize(doc: &SmlDocument, settings: &SmlComparerSettings) -> Result<WorkbookSignature> {
        ...
        let workbook_root = workbook_doc.root()
        ...
        // Process each sheet
        let sheets_elem = workbook_doc.elements_by_name(workbook_root, &S::sheets())
            .next()
            .ok_or_else(|| RedlineError::InvalidPackage { 
        ...
        for sheet_elem in workbook_doc.elements_by_name(sheets_elem, &S::sheet()) {
            let sheet_data = workbook_doc.get(sheet_elem)
                .ok_or_else(|| RedlineError::InvalidPackage { 
                    message: "Sheet element has no data".to_string() 
                })?;
            
            let attrs = sheet_data.attributes()
                .ok_or_else(|| RedlineError::InvalidPackage { 
                    message: "Sheet has no attributes".to_string() 
                })?;
            
            let sheet_name = attrs.iter()
        ...
        // Get defined names
        if let Some(defined_names_elem) = workbook_doc.elements_by_name(workbook_root, &S::definedNames()).next() {
                ...
                if let Some(dn_data) = workbook_doc.get(dn_elem) {
                    if let Some(attrs) = dn_data.attributes() {
                        if let Some(name_attr) = attrs.iter().find(|a| a.name.local_name == "name") {
                            ...
                            let value = get_text_content(&workbook_doc, dn_elem).unwrap_or_default();
                            if !name.is_empty() {
    ...
    fn resolve_worksheet_path(pkg: &OoxmlPackage, r_id: &str) -> Result<String> {
        ...
        let rel_name = XName::new("http://schemas.openxmlformats.org/package/2006/relationships", "Relationship");
                ...
                if let Some(attrs) = rel_data.attributes() {
                    if let Some(id_attr) = attrs.iter().find(|a| a.name.local_name == "Id") {
                        if id_attr.value == r_id {
                            if let Some(target_attr) = attrs.iter().find(|a| a.name.local_name == "Target") {
    ...
    fn canonicalize_cell(
        ...
        // Get formula
        let formula = ws_doc.elements_by_name(cell_elem, &S::f())
            .next()
            .and_then(|f_elem| get_text_content(ws_doc, f_elem));
        
        // Get format
        let style_index = cell_data.attributes()
        ...
        let content_hash = CellSignature::compute_hash(
    ...
    fn resolve_value(
        ...
        let cell_type = cell_data.attributes()
        ...
        Ok(Some(match cell_type {
            ...
            "str" => raw_value,
            "b" => if raw_value == "1" { "TRUE".to_string() } else { "FALSE".to_string() },
    ...
    fn get_style_info(pkg: &OoxmlPackage) -> Result<StyleInfo> {
        ...
        let styles_root = styles_doc.root()
            .ok_or_else(|| RedlineError::InvalidPackage { 
                message: "Styles has no root".to_string() 
            })?;
        
        // Get number formats
        if let Some(num_fmts_elem) = styles_doc.elements_by_name(styles_root, &S::numFmts()).next() {
        ...
        // Get borders
        if let Some(borders_elem) = styles_doc.elements_by_name(styles_root, &S::borders()).next() {
        ...
        // Get cell XFs
        if let Some(cell_xfs_elem) = styles_doc.elements_by_name(styles_root, &S::cellXfs()).next() {
    ...
    fn parse_fill(doc: &XmlDocument, fill_elem: NodeId) -> FillInfo {
        ...
        if let Some(pattern_fill) = doc.elements_by_name(fill_elem, &XName::new(S::NS, "patternFill")).next() {
            if let Some(pf_data) = doc.get(pattern_fill) {
                if let Some(attrs) = pf_data.attributes() {
                    fill.pattern = attrs.iter()
                        .find(|a| a.name.local_name == "patternType")
                        .map(|a| a.value.clone());
                }
                
                // Foreground color
                if let Some(fg_elem) = doc.elements_by_name(pattern_fill, &XName::new(S::NS, "fgColor")).next() {
                    if let Some(fg_data) = doc.get(fg_elem) {
                        if let Some(attrs) = fg_data.attributes() {
                            fill.fg_color = attrs.iter()
                                .find(|a| a.name.local_name == "rgb" || a.name.local_name == "theme")
                                .map(|a| a.value.clone());
                        }
                    }
                ...
                // Background color
                if let Some(bg_elem) = doc.elements_by_name(pattern_fill, &XName::new(S::NS, "bgColor")).next() {
                    if let Some(bg_data) = doc.get(bg_elem) {
                        if let Some(attrs) = bg_data.attributes() {
                            fill.bg_color = attrs.iter()
                                .find(|a| a.name.local_name == "rgb" || a.name.local_name == "theme")
                                .map(|a| a.value.clone());
                        }
                    }
    ...
    fn parse_border_side(doc: &XmlDocument, side_elem: NodeId) -> (Option<String>, Option<String>) {
        ...
        let style = side_data.and_then(|d| d.attributes())
            .and_then(|attrs| attrs.iter().find(|a| a.name.local_name == "style"))
            .map(|a| a.value.clone());
        
        let color = doc.elements_by_name(side_elem, &XName::new(S::NS, "color"))
            .next()
            .and_then(|color_elem| doc.get(color_elem))
            .and_then(|color_data| color_data.attributes())
            .and_then(|attrs| attrs.iter().find(|a| a.name.local_name == "rgb" || a.name.local_name == "theme"))
            .map(|a| a.value.clone());
    ...
    fn expand_style(style_index: usize, style_info: &StyleInfo) -> CellFormatSignature {
        ...
        // Get number format
        let number_format_code = xf.num_fmt_id
    ...
    fn compute_row_signatures(signature: &mut WorksheetSignature, settings: &SmlComparerSettings) {
        for &row_index in &signature.populated_rows {
            ...
            let row_content: Vec<String> = sampled.iter()
                .map(|c| c.resolved_value.as_deref().unwrap_or(""))
                .map(|s| s.to_string())
                .collect();
            
            let row_content_str = row_content.join("|");
            signature.row_signatures.insert(row_index, Self::compute_quick_hash(&row_content_str));
        }
    }
    
    /// Compute hash signatures for each column to enable LCS-based alignment.
    fn compute_column_signatures(signature: &mut WorksheetSignature, settings: &SmlComparerSettings) {
        for &col_index in &signature.populated_columns {
            ...
            let col_content: Vec<String> = sampled.iter()
                .map(|c| c.resolved_value.as_deref().unwrap_or(""))
                .map(|s| s.to_string())
                .collect();
            
            let col_content_str = col_content.join("|");
            signature.column_signatures.insert(col_index, Self::compute_quick_hash(&col_content_str));
        }
    }
    ...
    fn extract_comments(
        ...
        let rel_name = XName::new("http://schemas.openxmlformats.org/package/2006/relationships", "Relationship");
        let comments_type = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/comments";
        ...
        if let Some(authors_elem) = comments_doc.elements_by_name(comments_root, &S::authors()).next() {
            let mut index = 0;
        ...
        // Get comments
        if let Some(comment_list_elem) = comments_doc.elements_by_name(comments_root, &S::commentList()).next() {
            for comment_elem in comments_doc.elements_by_name(comment_list_elem, &S::comment()) {
                if let Some(comment_data) = comments_doc.get(comment_elem) {
                    if let Some(attrs) = comment_data.attributes() {
                        ...
                        let mut text = String::new();
                        if let Some(text_elem) = comments_doc.elements_by_name(comment_elem, &S::text()).next() {
                            for descendant in comments_doc.descendants(text_elem) {
                                if let Some(d_data) = comments_doc.get(descendant) {
                                    if let Some(name) = d_data.name() {
                                        if name == &S::t() {
                                            if let Some(content) = get_text_content(&comments_doc, descendant) {
                                                text.push_str(&content);
                                            }
                                        }
                                    }
                        ...
                        signature.comments.insert(cell_ref.clone(), CommentSignature {
    ...
    fn extract_data_validations(
        ...
        let data_validations_elem = ws_doc.elements_by_name(ws_root, &S::dataValidations()).next();
            ...
            if let Some(dv_data) = ws_doc.get(dv_elem) {
                if let Some(attrs) = dv_data.attributes() {
                    ...
                    for range in sqref.split_whitespace() {
                        ...
                        signature.data_validations.insert(key, DataValidationSignature {
        ...
        for hl_elem in ws_doc.elements_by_name(hyperlinks_elem.unwrap(), &S::hyperlink()) {
            if let Some(hl_data) = ws_doc.get(hl_elem) {
                if let Some(attrs) = hl_data.attributes() {
                    ...
                    if target.is_empty() {
                        if let Some(location_attr) = attrs.iter().find(|a| a.name.local_name == "location") {
                            target = location_attr.value.clone();
                        }
                    }
                    
                    let display = attrs.iter()
                        .find(|a| a.name.local_name == "display")
                        .map(|a| a.value.clone());
                    
                    let tooltip = attrs.iter()
                        .find(|a| a.name.local_name == "tooltip")
                        .map(|a| a.value.clone());
                    
                    signature.hyperlinks.insert(cell_ref.clone(), HyperlinkSignature {
                        cell_address: cell_ref,
                        target,
                        display,
                        tooltip,
...
mod tests {
    ...
    fn test_parse_cell_reference() {
        assert_eq!(SmlCanonicalizer::parse_cell_reference("A1").unwrap(), (1, 1));
        assert_eq!(SmlCanonicalizer::parse_cell_reference("B2").unwrap(), (2, 2));
        assert_eq!(SmlCanonicalizer::parse_cell_reference("Z26").unwrap(), (26, 26));
        assert_eq!(SmlCanonicalizer::parse_cell_reference("AA1").unwrap(), (27, 1));
        assert_eq!(SmlCanonicalizer::parse_cell_reference("AB10").unwrap(), (28, 10));
    }
