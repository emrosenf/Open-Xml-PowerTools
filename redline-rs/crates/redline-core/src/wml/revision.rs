...
pub fn create_run_property_change(
    ...
    doc.add_child(
        parent,
        XmlNodeData::element_with_attrs(W::rPrChange(), attrs),
    )
}
...
pub fn create_paragraph_property_change(
    ...
    doc.add_child(
        parent,
        XmlNodeData::element_with_attrs(W::pPrChange(), attrs),
    )
}
...
pub fn is_revision_element(name: &XName) -> bool {
    name.namespace.as_deref() == Some(W::NS) && matches!(name.local_name.as_str(), "ins" | "del")
}
...
mod tests {
    ...
    fn create_text_run_works() {
        ...
        assert_eq!(
            run_data.name().map(|n| &n.local_name),
            Some(&"r".to_string())
        );
    }
}
