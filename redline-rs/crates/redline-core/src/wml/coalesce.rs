...
pub fn strip_pt_attributes(doc: &mut XmlDocument, root: NodeId) {
    ...
    for node in nodes {
        let Some(node_data) = doc.get_mut(node) else {
            continue;
        };
        let Some(attrs) = node_data.attributes_mut() else {
            continue;
        };
...
pub fn coalesce(
    ...
    for (prefix, uri) in standard_namespaces {
        if !attrs.iter().any(|a| {
            a.name.local_name == prefix
                && a.name.namespace.as_deref() == Some("http://www.w3.org/2000/xmlns/")
        }) {
...
fn mark_content_transform(doc: &mut XmlDocument, node: NodeId, settings: &RevisionSettings) {
    ...
    let Some(name) = data.name().cloned() else {
        return;
    };
...
fn collect_run_descendant_statuses(
    doc: &XmlDocument,
    node: NodeId,
...
fn handle_ppr_element(doc: &mut XmlDocument, ppr: NodeId, settings: &RevisionSettings) {
    ...
    let rpr = doc.children(ppr).find(|&c| {
        doc.get(c)
            .and_then(|d| d.name())
            .map(|n| n.namespace.as_deref() == Some(W::NS) && n.local_name == "rPr")
            .unwrap_or(false)
...
fn is_props_element(doc: &XmlDocument, node: NodeId) -> bool {
    doc.get(node)
        .and_then(|d| d.name())
        .map(|n| n.namespace.as_deref() == Some(W::NS) && n.local_name.ends_with("Pr"))
        .unwrap_or(false)
}
...
pub fn coalesce_adjacent_runs_with_identical_formatting(
    ...
    let keys: Vec<String> = children
        .iter()
        .map(|&c| get_consolidation_key(doc, c))
        .collect();
...
fn collect_descendants_by_name(
    doc: &XmlDocument,
    node: NodeId,
    ns: &str,
    local: &str,
        ...
        for child in doc.children(current) {
            if let Some(data) = doc.get(child) {
                if let Some(name) = data.name() {
                    ...
                    if name.namespace.as_deref() == Some(target_ns)
                        && name.local_name == target_local
                    {
                        result.push(child);
                    }
                }
...
fn get_run_consolidation_key(doc: &XmlDocument, run: NodeId) -> String {
    ...
    let non_rpr_children: Vec<_> = doc
        .children(run)
...
fn get_ins_consolidation_key(doc: &XmlDocument, ins: NodeId) -> String {
    ...
    let grandchildren: Vec<_> = doc.children(ins).flat_map(|c| doc.children(c)).collect();

    let non_rpr_grandchildren_count = grandchildren
        .iter()
    ...
    let has_t_grandchild = grandchildren
        .iter()
...
fn get_del_consolidation_key(doc: &XmlDocument, del: NodeId) -> String {
    ...
    let r_children: Vec<_> = doc
        .children(del)
        .filter(|&c| is_element_named(doc, c, W::NS, "r"))
        .collect();

    let grandchildren: Vec<_> = r_children.iter().flat_map(|&r| doc.children(r)).collect();
    ...
    let has_del_text = grandchildren
        .iter()
    ...
    let rpr_strings: String = r_children
        .iter()
...
fn collect_text_from_group(doc: &XmlDocument, group: &[NodeId], _key: &str) -> String {
    ...
    for &node in group {
        ...
        for desc in doc.descendants(node) {
            if let Some(data) = doc.get(desc) {
                if let Some(name) = data.name() {
                    let is_text_element = (name.namespace.as_deref() == Some(W::NS))
                        && (name.local_name == "t"
                            || name.local_name == "delText"
                            || name.local_name == "instrText");
...
fn merge_run_group(
    ...
    let text_elem_name = if is_instr_text {
        W::instrText()
    } else {
        W::t()
    };
    ...
    let text_elem = doc.add_child(
        new_run,
        XmlNodeData::element_with_attrs(text_elem_name, text_attrs),
    );
...
fn merge_ins_group(
    ...
    let r_attrs = first_r
        .map(|r| get_element_attributes(doc, r))
        .unwrap_or_default();
...
fn merge_del_group(
    ...
    let dt_elem = doc.add_child(
        new_r,
        XmlNodeData::element_with_attrs(W::delText(), dt_attrs),
    );
...
fn has_child_element(doc: &XmlDocument, parent: NodeId, ns: &str, local: &str) -> bool {
    doc.children(parent)
        .any(|c| is_element_named(doc, c, ns, local))
}
...
fn has_attribute(doc: &XmlDocument, node: NodeId, ns: &str, local: &str) -> bool {
    doc.get(node)
        ...
        .map(|attrs| {
            attrs
                .iter()
                .any(|a| a.name.namespace.as_deref() == Some(ns) && a.name.local_name == local)
        })
        .unwrap_or(false)
}
...
fn merge_nodes(doc: &mut XmlDocument, nodes: &[NodeId]) -> NodeId {
    ...
    for &node in nodes {
        combined_text.push_str(&get_text_from_run_container(doc, node));
    }
...
fn get_text_from_run_container(doc: &XmlDocument, node: NodeId) -> String {
    let run = if doc
        .get(node)
        .and_then(|d| d.name())
        .map(|n| n.local_name == "r")
        .unwrap_or(false)
    {
        node
    } else {
        find_child_by_name(doc, node, W::NS, "r").unwrap_or(node)
    };
    let t_node = doc
        .children(run)
        .find(|&c| is_t(doc, c) || is_del_text(doc, c));
    if let Some(t) = t_node {
        if let Some(XmlNodeData::Text(txt)) = doc.children(t).next().and_then(|c| doc.get(c)) {
            return txt.clone();
        }
    }
    String::new()
...
fn set_text_in_run_container(doc: &mut XmlDocument, node: NodeId, text: &str) {
    ...
    if let Some(t) = t_node {
        ...
        if let Some(XmlNodeData::Text(ref mut txt)) = doc.get_mut(text_node) {
            *txt = text.to_string();
        }
    }
}
    ...
    for (group_key, start, end) in grouped {
        if group_key.is_empty() {
            ...
            */
            ...
            let is_inside_vml = direct_children
                .first()
                .map(|a| is_inside_vml_content(a, level))
                .unwrap_or(false);
            let grouped_direct = group_adjacent_by_correlation_ranges(
                direct_children,
                level,
                is_inside_vml,
                settings,
        ...
        let ancestor_being_constructed =
            get_ancestor_element_for_level(first_atom, level, group_atoms);
        ...
        let grouped_children =
            group_adjacent_by_correlation_ranges(group_atoms, level, is_inside_vml, settings);

        match ancestor_name.as_str() {
            "p" => reconstruct_paragraph(
                doc,
                parent,
                &group_key,
            ...
            "drawing" => reconstruct_drawing_elements(
                doc,
                parent,
                group_atoms,
                &grouped_children,
            ...
            "oMath" | "oMathPara" => reconstruct_element(
                doc,
                parent,
                &group_key,
                &ancestor_being_constructed,
            ...
            elem if ALLOWABLE_RUN_CHILDREN.contains(&elem) => reconstruct_allowable_run_children(
                doc,
                parent,
                &ancestor_being_constructed,
                group_atoms,
            ...
            "pict" | "shape" | "rect" | "group" | "shapetype" | "oval" | "line" | "arc"
            | "curve" | "polyline" | "roundrect" => reconstruct_vml_element(
                doc,
                parent,
                &group_key,
                &ancestor_being_constructed,
...
fn get_uniform_status(atoms: &[ComparisonUnitAtom]) -> Option<ComparisonCorrelationStatus> {
    ...
    if matches!(
        status,
        ComparisonCorrelationStatus::Inserted | ComparisonCorrelationStatus::Deleted
    ) {
...
fn reconstruct_paragraph(
    ...
    let has_rsid_del = ancestor
        .attributes
        .iter()
        .any(|a| a.name.namespace.as_deref() == Some(W::NS) && a.name.local_name == "rsidDel");
    ...
    for (key, start, end) in grouped_children {
        ...
        if spl.get(0) == Some(&"") {
            for gcc in group_atoms {
                ...
                if is_inside_vml && spl.get(1) == Some(&"Inserted") {
                    continue;
                }
...
fn copy_element_from_xml(
    ...
    if element_xml.len() < 100 {
        eprintln!(
            "WARN copy_element_from_xml: element_xml very short: {}",
            element_xml
        );
    }
    ...
    eprintln!(
        "DEBUG copy_element_from_xml: parsed node has {} descendants",
        child_count
    );
    if child_count == 0 {
        eprintln!(
            "  WARNING: No descendants! element_xml first 200 chars: {}",
            &element_xml.chars().take(200).collect::<String>()
        );
    }
...
fn create_content_element(
    ...
    match &atom.content_element {
        ...
        ContentElement::CarriageReturn => {
            ...
            if !status.is_empty() {
                doc.set_attribute(cr, &pt_status(), status);
            }
            Some(cr)
        }
        ContentElement::Tab => {
        ...
        ContentElement::PositionalTab {
            ...
            if !alignment.is_empty() {
                attrs.push(XAttribute::new(W::alignment(), alignment));
            }
            if !relative_to.is_empty() {
                attrs.push(XAttribute::new(W::relativeTo(), relative_to));
            }
            if !leader.is_empty() {
                attrs.push(XAttribute::new(W::leader(), leader));
            }
            if !status.is_empty() {
            ...
            if element_xml.is_empty() {
                ...
                if let Some(new_node) = parse_xml_fragment(doc, element_xml) {
                    ...
                    if child_count == 0 {
                        ...
                        eprintln!(
                            "  element_xml first 300 chars: {}",
                            &element_xml.chars().take(300).collect::<String>()
                        );
                    }
                    if !status.is_empty() {
                        doc.set_attribute(new_node, &pt_status(), status);
        ...
        ContentElement::Picture { element_xml, .. } => {
            ...
            if element_xml.is_empty() {
                ...
                if !status.is_empty() {
                    doc.set_attribute(pict, &pt_status(), status);
                }
                Some(pict)
            } else {
        ...
        ContentElement::Math { element_xml, .. } => {
            ...
            if element_xml.is_empty() {
                let math = doc.new_node(XmlNodeData::element(XName::new(
                    "http://schemas.openxmlformats.org/officeDocument/2006/math",
                    "oMath",
                )));
                if !status.is_empty() {
                    doc.set_attribute(math, &pt_status(), status);
                }
                Some(math)
            } else {
                if let Ok(source_doc) = parse(element_xml) {
                    if let Some(source_root) = source_doc.root() {
                        ...
                        if let Some(new_node) =
                            deep_copy_node(doc, temp_parent, &source_doc, source_root)
                        {
                            doc.detach(new_node);
                            if !status.is_empty() {
                                doc.set_attribute(new_node, &pt_status(), status);
        ...
        ContentElement::CommentRangeStart { id } => {
            ...
            Some(doc.new_node(XmlNodeData::element_with_attrs(
                W::commentRangeStart(),
                attrs,
            )))
        }
        ...
        ContentElement::Symbol { font, char_code } => {
            let mut attrs = Vec::new();
            if !font.is_empty() {
                attrs.push(XAttribute::new(XName::new(W::NS, "font"), font));
            }
            if !char_code.is_empty() {
                attrs.push(XAttribute::new(XName::new(W::NS, "char"), char_code));
            }
            if !status.is_empty() {
                attrs.push(XAttribute::new(pt_status(), status));
            }
            ...
            if settings.track_formatting_changes {
                if atom.correlation_status == ComparisonCorrelationStatus::FormatChanged {
                    format!(
                        ...
                        ancestor_unid,
                        status_str,
                        atom.formatting_change_rpr_before_signature
                            .as_deref()
                            .unwrap_or("<null>"),
...
fn get_ancestor_element_for_level(
    ...
    AncestorElementInfo {
        ...
        attributes: first_atom.ancestor_elements[level]
            .attributes
            .as_ref()
            .clone(),
...
mod tests {
    ...
    fn test_format_date_for_key() {
        ...
        assert_eq!(
            format_date_for_key("2023-01-15T10:30:00Z"),
            "2023-01-15T10:30:00"
        );
        // Date with offset
        assert_eq!(
            format_date_for_key("2023-01-15T10:30:00+05:00"),
            "2023-01-15T10:30:00"
        );
        // Short date
    ...
    fn test_get_consolidation_key_ins_element() {
        let mut doc = XmlDocument::new();
        let ins = doc.add_root(XmlNodeData::element_with_attrs(
            W::ins(),
            vec![
    ...
    fn test_get_consolidation_key_del_element() {
        let mut doc = XmlDocument::new();
        let del = doc.add_root(XmlNodeData::element_with_attrs(
            W::del(),
            vec![
                XAttribute::new(W::id(), "1"), // w:id MUST come first per ECMA-376
                XAttribute::new(W::author(), "Test Author"),
    ...
    fn test_coalesce_adjacent_runs_merges_identical_runs() {
        ...
        let t_elem = doc
            .children(merged_run)
            .find(|&c| is_element_named(&doc, c, W::NS, "t"));
        ...
        let rpr_count_before: usize = std::iter::once(root)
            ...
            .filter(|&n| {
                doc.get(n)
                    .and_then(|d| d.name())
                    .map(|name| {
                        name.namespace.as_deref() == Some(W::NS) && name.local_name == "rPr"
                    })
                    .unwrap_or(false)
            })
            .count();
